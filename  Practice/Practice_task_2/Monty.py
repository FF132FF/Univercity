...
def monty_hall_paradox(iterations: int):
    '''
    Функция проверяет и доказывает: правда ли, что вероятность победы увеличится при принятии предложения ведущего
    перевыбрать дверь; а так же можно проверить какой будет вероятность победы, если не принимать предложение ведущего
    и не менять дверь. Возвращает разделитель (весь вывод отображается внутри функции, поэтому возвращает его)
